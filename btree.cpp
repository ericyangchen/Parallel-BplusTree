...
int find_range(node *const root, int key_start, int key_end, bool verbose,
  ...
  while (n != NULL) {
    for (; i < n->num_keys; i++) {
      if(n->keys[i] > key_end)return num_found;
      if(returned_keys != NULL) returned_keys[num_found] = n->keys[i];
      if(returned_pointers != NULL) returned_pointers[num_found] = n->pointers[i];
      num_found++;
    }
...
int find_range_maxvalue(node *const root, int key_start, int key_end, bool verbose) {
  ...
  while (n != NULL) {
    for (; i < n->num_keys; i++) {
      if(n->keys[i] > key_end)return max;
      max = max > *((int *) n->pointers[i]) ? max : *((int *) n->pointers[i]);
    }
    n = (node *) n->pointers[order - 1];
    i = 0;
  }
...
node *coalesce_nodes(node *root, node *n, node *neighbor, int neighbor_index, int k_prime) {
  ...
  root = remove_entry(root, n->parent, k_prime, n);
  free(n->keys);
  free(n->pointers);
  free(n);
  return root;
...
node *remove (node *root, int key) {
  ...
  if (key_record != NULL && key_leaf != NULL) {
    root = remove_entry(root, key_leaf, key, key_record);
    free(key_record);
  }
  return root;
}
